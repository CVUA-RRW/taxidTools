...
# -*- coding: utf-8 -*-
"""
Provides the Taxdump class to work with the NCBI Taxonomy informations from the
taxdump files.
Supports conversion of taxid numbers to rank or names, finding parents,
retrieving ancestry and finding lowest common
...
class _ReadOnlyDict(UserDict):
    ...
    Can still be updated by using _ReadOnlyDict.data[key] = val (Should only be
...
class Taxdump(object):
    ...
    def __init__(self, rankedlineage_dmp, nodes_dmp):
        ...
        nodes_dmp: str
            Path to nodes.dmp
        """
        # useful data will be stored in a dict as
        # {taxid: (name, rank, parent_taxid)}
        # the aim here is to keep data storage at a minimum to use as little
        # memory as nescessary without the hurdle of indexing the files
    ...
    def lowestCommonNode(self, taxid_list):
        ...
        while size <= 1:
            ...
            # If one accession is an ancestor of all others, it will run out of
            # entries and raise an IdenxError
    ...
    def _flip_dict(self):
        ...
        for key, value in self._data.items():
            # The flipped dictionnary will not contain rank and parent
            # informations!
            flipped[value[0]] = key
